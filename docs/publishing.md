...
- **`verify`** runs the tests and builds the package. It holds **no
  credentials** and has no `id-token` permission, so nothing here can
  authenticate to Google. This is also where the two GitHub-authored actions
  (`checkout`, `setup-node`) run.
- **`publish`** requests the OIDC token and talks to the store. It runs only
  after `verify` passes and only after the environment gate is approved.
...
and they run in the job that holds nothing.
